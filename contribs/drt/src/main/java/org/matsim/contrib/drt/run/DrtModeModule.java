...
public final class DrtModeModule extends AbstractDvrpModeModule {
	...
	public void install() {
		...
		if (drtCfg.getRebalancingParams().isPresent()) {
			AbstractRebalancingParams rebalancingParams = drtCfg.getRebalancingParams().get();
			if (rebalancingParams instanceof MinCostFlowRebalancingParams) {
				install(new DrtModeMinCostFlowRebalancingModule(drtCfg));
			} else {
				throw new RuntimeException("Unsupported rebalancing params: " + rebalancingParams);
			}
		} else {
