 ...
 * @author michalm
 */
public abstract class AbstractRebalancingParams extends ReflectiveConfigGroup {
	...
	protected AbstractRebalancingParams(String paramSetName) {
		super(paramSetName);
	}
	...
	protected void checkConsistency(Config config) {
		...
		if (getMinServiceTime() <= getMaxTimeBeforeIdle()) {
			throw new RuntimeException(AbstractRebalancingParams.MIN_SERVICE_TIME
					+ " must be greater than "
					+ AbstractRebalancingParams.MAX_TIME_BEFORE_IDLE);
		}

		Verify.verify(getRebalancingZonesGeneration() != RebalancingZoneGeneration.ShapeFile
